...
PPM::PPM(int a, int b, int c) {
	...
	try {
		...
		throw_error("insufficient memory while creating render matrix");
		std::exit(1);
	}
...
int PPM::write(const std::string path) const {
	...
	//size_t s = sizeof matrix / sizeof matrix[0];
...
std::string PPM::generate_headers() const {
	std::string headers;
...
void PPM::set_pos(const int x, const int y, const unsigned int r[]) const {
	int n = offset(x, y);
...
void PPM::set_pos(const int x, const int y, const val n[]) const {
...
void PPM::set_pos(const int x, const int y, const int z, const unsigned int n) const {
	if (n > color_depth) {
...
void PPM::set_pos(const int x, const int y, const int z, const val n) const {
	if (n > 1.0) {
...
int PPM::offset(const int x, const int y) const {
	return (n) - ((y * width)  + x ) * 3;
}

int PPM::offset(const int x, const int y, const int z) const {
	return (n) - ((y * width) + x) * 3 + z;
}

void PPM::debug_matrix() const {
