...
void render_row(const unsigned int j) {
...
int main() {
	...
	unsigned int n = HEIGHT / RENDER_THREADS;

	std::thread pool[RENDER_THREADS];
	...
	for (unsigned int j = 0; j < n; j++) {
		if (j < n - 1) {
			for (unsigned int t = 0; t < RENDER_THREADS; t++) {
				if (!pool[t].joinable()) {
					pool[t] = std::thread(render_row, 4 * j + t);
				}
			}
			...
			update_progress_bar("+ Rendering", ((val)(j) * 4) / HEIGHT);
		}
